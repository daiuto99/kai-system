...
class AllowPath(unittest.TestCase):
    def test_enrolled_online_peer_single_cgnat_allows(self):
        st = status(peers={"a": node("ntzBBuNMsE11CNTRL", ["100.106.160.41", "fd7a:115c::1"],
...
class DenyPaths(unittest.TestCase):
    ...
    def test_enrolled_id_absent(self):
        ...
        self.assertFalse(tg.evaluate_target("71-kai-mini", ALLOW, st).allowed)

    def test_hostname_spoof_wrong_id_denied(self):
        st = status(peers={"evil": node("nEVILimposter00", ["100.106.160.41"], online=True,
    ...
    def test_online_int_one_denied(self):
        st = status(peers={"a": node("ntzBBuNMsE11CNTRL", ["100.106.160.41"], online=1)})
        self.assertFalse(tg.evaluate_target("71-kai-mini", ALLOW, st).allowed)
    ...
    def test_peer_absent_online_key_denied(self):
        ...
        st = status(peers={"a": {"ID": "ntzBBuNMsE11CNTRL", "TailscaleIPs": ["100.106.160.41"]}})
        self.assertFalse(tg.evaluate_target("71-kai-mini", ALLOW, st).allowed)

    def test_duplicate_id_in_status_denied(self):
        st = status(peers={"a": node("ntzBBuNMsE11CNTRL", ["100.106.160.41"], online=True),
                           "b": node("ntzBBuNMsE11CNTRL", ["100.106.160.42"], online=True)})
        d = tg.evaluate_target("71-kai-mini", ALLOW, st)
        self.assertFalse(d.allowed)
    ...
    def test_ip_outside_cgnat_denied(self):
        for ip in ["100.32.0.1", "100.63.255.255", "100.128.0.1", "101.0.0.1", "10.0.0.5"]:
            st = status(peers={"a": node("ntzBBuNMsE11CNTRL", [ip])})
            self.assertFalse(tg.evaluate_target("71-kai-mini", ALLOW, st).allowed, ip)

    def test_cgnat_boundaries_allow(self):
        for ip in ["100.64.0.0", "100.127.255.255"]:
            st = status(peers={"a": node("ntzBBuNMsE11CNTRL", [ip], online=True)})
            self.assertTrue(tg.evaluate_target("71-kai-mini", ALLOW, st).allowed, ip)

    def test_multiple_cgnat_ambiguous_denied(self):
        st = status(peers={"a": node("ntzBBuNMsE11CNTRL", ["100.64.0.1", "100.64.0.2"], online=True)})
        d = tg.evaluate_target("71-kai-mini", ALLOW, st)
        self.assertFalse(d.allowed)
        self.assertIn("ambiguous", d.reason)

    def test_tailscaleips_dict_denied(self):
        st = status(peers={"a": {"ID": "ntzBBuNMsE11CNTRL", "Online": True,
                                 "TailscaleIPs": {"0": "100.106.160.41"}}})
        self.assertFalse(tg.evaluate_target("71-kai-mini", ALLOW, st).allowed)

    def test_tailscaleips_int_denied(self):
        st = status(peers={"a": {"ID": "ntzBBuNMsE11CNTRL", "Online": True, "TailscaleIPs": 5}})
        self.assertFalse(tg.evaluate_target("71-kai-mini", ALLOW, st).allowed)

    def test_nonstring_ip_element_denied(self):
        st = status(peers={"a": {"ID": "ntzBBuNMsE11CNTRL", "Online": True,
                                 "TailscaleIPs": [123, "100.106.160.41"]}})
        self.assertFalse(tg.evaluate_target("71-kai-mini", ALLOW, st).allowed)

    def test_malformed_ip_element_denies_whole_decision(self):
        st = status(peers={"a": {"ID": "ntzBBuNMsE11CNTRL", "Online": True,
                                 "TailscaleIPs": ["not-an-ip", "100.106.160.41"]}})
        self.assertFalse(tg.evaluate_target("71-kai-mini", ALLOW, st).allowed)

    def test_invalid_node_id_format_in_allowlist_denied(self):
        bad_allow = {"71-kai-mini": "not a node id!"}
        st = status(peers={"a": node("not a node id!", ["100.106.160.41"], online=True)})
        self.assertFalse(tg.evaluate_target("71-kai-mini", bad_allow, st).allowed)
    ...
    def test_matched_entry_missing_id_denied(self):
        st = status(peers={"a": {"Online": True, "TailscaleIPs": ["100.106.160.41"]}})  # no ID
        self.assertFalse(tg.evaluate_target("71-kai-mini", ALLOW, st).allowed)
    ...
    def test_self_and_peer_duplicate_id_ambiguous_denied(self):
        st = status(self_entry=node("ntzBBuNMsE11CNTRL", ["100.106.160.41"], online=None),
                    peers={"a": node("ntzBBuNMsE11CNTRL", ["100.106.160.42"], online=True)})
        d = tg.evaluate_target("71-kai-mini", ALLOW, st)
        self.assertFalse(d.allowed)
        self.assertIn("ambiguous", d.reason)
    ...
    def test_non_dict_peer_value_denied(self):
        st = {"BackendState": "Running",
              "Peer": {"good": node("ntzBBuNMsE11CNTRL", ["100.106.160.41"], online=True),
                       "garbage": "not-an-object"}}
        self.assertFalse(tg.evaluate_target("71-kai-mini", ALLOW, st).allowed)

    def test_dirty_allowlist_duplicate_ids_denied_inline(self):
        dirty = {"alias1": "ntzBBuNMsE11CNTRL", "alias2": "ntzBBuNMsE11CNTRL"}
        st = status(peers={"a": node("ntzBBuNMsE11CNTRL", ["100.106.160.41"], online=True)})
    ...
    def test_dirty_allowlist_invalid_sibling_id_denies_valid_name(self):
        dirty = {"71-kai-mini": "ntzBBuNMsE11CNTRL", "poison": "42"}
        st = status(peers={"a": node("ntzBBuNMsE11CNTRL", ["100.106.160.41"], online=True)})
        self.assertFalse(tg.evaluate_target("71-kai-mini", dirty, st).allowed)
...
class LoadAllowlist(unittest.TestCase):
    ...
    def test_confirmed_enrollment_loads(self):
        p = self._write({"enrollment_status": "confirmed",
                         "nodes": {"71-kai-mini": "ntzBBuNMsE11CNTRL"}})
        self.assertEqual(tg.load_allowlist(p), {"71-kai-mini": "ntzBBuNMsE11CNTRL"})

    def test_duplicate_ids_return_empty(self):
        p = self._write({"enrollment_status": "confirmed",
                         "nodes": {"a": "ntzBBuNMsE11CNTRL", "b": "ntzBBuNMsE11CNTRL"}})
        self.assertEqual(tg.load_allowlist(p), {})
    ...
    def test_duplicate_json_keys_return_empty(self):
        ...
        p = self._write('{"enrollment_status":"confirmed","nodes":'
                        '{"71-kai-mini":"ntzBBuNMsE11CNTRL","71-kai-mini":"nEVILimposter00"}}')
        self.assertEqual(tg.load_allowlist(p), {})
