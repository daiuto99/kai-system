...
`m1smoke` Qdrant collection and append two verified facts: one scoped to
`project=alpha`, one to `project=beta`, both with `task_type=m1-scope`. It then
...
The expected results are alpha-only, beta-only, then both facts for the
unscoped call. The final union is the no-default proof: current unscoped
`facts_for()` behavior applies no project filter. Unit coverage also asserts
that absent `project` and `task_type` keys are omitted from the outbound
assemble payload rather than sent as defaults.
