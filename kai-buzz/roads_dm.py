#!/usr/bin/env python3
"""Roads as a NIP-17 DM agent — the always-on 1:1 advisor DM (ported verbatim from sky_dm.py,
...
KAI (agents_bridge.py Roads): one implementation per path (M-R2).
...
_secret_hex = open(os.path.join(ab.AGENT_DIR, "roads.key")).read().strip()
ROADS_KEYS = Keys.parse(_secret_hex)
ROADS_SIGNER = NostrSigner.keys(ROADS_KEYS)
ROADS_PUB_HEX = ROADS_KEYS.public_key().to_hex()
LEO_PUB = PublicKey.parse(ab.LEO_PUBKEY)
INTRO_MARKER = os.path.join(ab.AGENT_DIR, "roads_dm_intro_sent")

INTRO = ('Roads here — now a 1:1 direct message, always on from the server. Ask me about guitar tech, gear, tone and rigs anytime; I answer whether your Mac is open or asleep. Group gear chat stays in GearTalk.')


async def _wrap_json(receiver_pub, text):
    rumor = EventBuilder.private_msg_rumor(receiver_pub, text).build(ROADS_KEYS.public_key())
    wrap = await gift_wrap(ROADS_SIGNER, receiver_pub, rumor, [])
    return json.loads(wrap.as_json())


async def run():
    pk = ab.load_or_create_key("roads.key")            # coincurve key, for NIP-42 auth
    ...
    ab.log("roads-dm", "Roads DM agent · pubkey", ROADS_PUB_HEX, "· connect", ab.CONNECT_URL)
    async with websockets.connect(ab.CONNECT_URL, max_size=2 ** 20) as ws:
        await ab.authenticate(ws, pk)
        async with send_lock:
            await ws.send(json.dumps(["REQ", "dm", {"kinds": [GIFT_WRAP_KIND], "#p": [ROADS_PUB_HEX],
        ...
        if not os.path.exists(INTRO_MARKER):
            try:
                ...
                ab.log("roads-dm", "intro DM sent to Leo")
            except Exception as e:
                ab.log("roads-dm", f"intro send failed: {e}")
        ab.log("roads-dm", "online — listening for Leo's DMs")
        seen = set()
        async for raw in ws:
            m = json.loads(raw)
            if m[0] == "AUTH":
            ...
            seen.add(wrap_ev["id"])
            try:
                uw = await UnwrappedGift.from_gift_wrap(ROADS_SIGNER, Event.from_json(json.dumps(wrap_ev)))
                sender_hex = uw.sender().to_hex()
                text = uw.rumor().content()
            except Exception as e:
                ab.log("roads-dm", f"unwrap failed: {e}")
                continue
            if sender_hex == ROADS_PUB_HEX:
                continue    # skip our own self-copies
            ab.log("roads-dm", f"<< {sender_hex[:8]}: {text[:80]}")
            since_ts = await asyncio.to_thread(ab.latest_dm_log_ts, "roads")
            try:
                reply = await asyncio.to_thread(ab.call_council, "roads", text, "roads-dm:" + sender_hex[:16])
                ...
                reply = f"(Roads ran into an error handling that: {e})"
            try:
                await send_dm(ws, uw.sender(), reply)
                ab.log("roads-dm", f">> {reply[:100]}")
            except Exception as e:
                ab.log("roads-dm", f"reply send failed: {e}")
            # AR-3/P-2 ready-notify: Roads runs async on the mini (~90s). The ack
            ...
            async def _deliver_async_answer(_r=uw.sender(), _m=text, _since=since_ts, _ack=reply):
                ans = await asyncio.to_thread(ab.poll_async_answer, "roads", _m, _since)
                if ans and ans.strip() and ans.strip() != _ack.strip():
                    try:
                        await send_dm(ws, _r, ans)
                        ab.log("roads-dm", f">> [ready] {ans[:100]}")
                    except Exception as e:
                        ab.log("roads-dm", f"ready-notify send failed: {e}")
                elif ans is None:
                    ab.log("roads-dm", "ready-notify: no async answer landed before timeout")
