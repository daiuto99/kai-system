...
KAI_TOOLS = [
    ...
    {"name": "wordpress_create_page", "description": "Create a full HTML/CSS page on a WordPress site. Use template=kai-blank for full design control (blank canvas, no theme wrapper). Status: draft or publish.", "input_schema": {"type": "object", "properties": {"site": {"type": "string"}, "title": {"type": "string"}, "content": {"type": "string", "description": "Full HTML+CSS content. For blank canvas pages include complete <style> blocks."}, "status": {"type": "string", "enum": ["draft", "publish"]}, "slug": {"type": "string"}, "template": {"type": "string", "description": "Use kai-blank for full design control, empty string for default theme template"}}, "required": ["site", "title", "content"]}},
    ...
    {"name": "wordpress_create_task", "description": "Add a WordPress design task to the queue (page to build, post to write, site to redesign).", "input_schema": {"type": "object", "properties": {"site": {"type": "string"}, "type": {"type": "string", "enum": ["page", "post", "redesign", "css"]}, "title": {"type": "string"}, "brief": {"type": "string"}, "priority": {"type": "string", "enum": ["high", "normal", "low"]}}, "required": ["site", "type", "title", "brief"]}},
    {"name": "wordpress_complete_task", "description": "Mark a WordPress design task as complete and record what was built.", "input_schema": {"type": "object", "properties": {"task_id": {"type": "string"}, "result": {"type": "string"}}, "required": ["task_id"]}},
    ...
    {"name": "wordpress_purge_varnish", "description": "Purge Cloudways Varnish cache for a WordPress site. Purges both the custom domain and the Cloudways FQDN host keys. Call after wordpress_write_file edits to cs.html, .htaccess, or any cacheable file.", "input_schema": {"type": "object", "properties": {"site": {"type": "string"}, "url_path": {"type": "string", "description": "URL path to purge, default '/'"}}, "required": ["site"]}},
