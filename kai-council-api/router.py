...
KAI_TOOLS = [
    ...
    {"name": "teardown_project", "description": "Full project teardown: removes from dashboard registry and moves vault folder to archived/. Use this when Leo says to delete or remove a project from everywhere.", "input_schema": {"type": "object", "properties": {"id": {"type": "string", "description": "project slug to tear down"}}, "required": ["id"]}},
    ...
    {"name": "setup_project", "description": "Full project creation pipeline: creates vault folder and Gmail drafts for external invitees. project_type='active' (default) registers on dashboard; project_type='idea' stores in vault/20_Projects/ideas/ only — no dashboard entry. BEHAVIOR: (1) For any person mentioned, call lookup_contact first, then lookup_google_contact if not found — never ask Leo for an email you can resolve. (2) If both lookups return nothing, use whatever info Leo provided and proceed. Never ask for clarification — act and report.", "input_schema": {"type": "object", "properties": {"id": {"type": "string", "description": "short slug, e.g. project-x"}, "name": {"type": "string"}, "description": {"type": "string"}, "advisor": {"type": "string"}, "project_type": {"type": "string", "enum": ["active", "idea"], "description": "active = live project on dashboard; idea = stored in ideas/ folder only"}, "status": {"type": "string", "enum": ["green","yellow","red"]}, "next": {"type": "string"}, "invite_contacts": {"type": "array", "items": {"type": "string"}, "description": "Internal contacts by name or ID"}, "external_invites": {"type": "array", "items": {"type": "object", "properties": {"email": {"type": "string"}, "name": {"type": "string"}}, "required": ["email"]}, "description": "External collaborators — Gmail drafts sent to each"}, "file_request_message": {"type": "string", "description": "Message to include in the invite about how to share files."}}, "required": ["id", "name"]}},
    ...
    {"name": "add_contact", "description": "Add a new person to the contacts registry.", "input_schema": {"type": "object", "properties": {"name": {"type": "string"}, "email": {"type": "string"}, "role": {"type": "string"}, "aliases": {"type": "array", "items": {"type": "string"}}}, "required": ["name"]}},
    ...
    {"name": "request_t2_approval", "description": "Queue any action that requires Leo's explicit approval. The request is delivered to Leo via Telegram (the sole approval surface).", "input_schema": {"type": "object", "properties": {"action": {"type": "string"}, "detail": {"type": "string"}}, "required": ["action", "detail"]}},
    {"name": "run_capability", "description": "Execute any registered KAI orchestrator capability by name. Use list_capabilities first to see available names and whether confirmation is required. For destructive capabilities (vault.write, session.close, workspace.sync) you must pass confirmed=true. Examples: run_capability('vault.read', {'path': 'file.md'}), run_capability('vault.write', {'path': 'note.md', 'content': '...'}, confirmed=true). Returns ok, data, error fields.", "input_schema": {"type": "object", "properties": {"capability": {"type": "string", "description": "Capability name, e.g. vault.read, vault.write, workspace.sync"}, "inputs": {"type": "object", "description": "Inputs specific to the capability (path, content, channel, etc.)"}, "confirmed": {"type": "boolean", "description": "Set true to confirm destructive operations (vault.write, session.close, workspace.sync)"}}, "required": ["capability"]}},
    ...
    {"name": "list_ideas", "description": "List Leo's loose ideas (slug + name + note) from the Project Console. Call this FIRST when Leo says 'work on <X>' / 'let's brainstorm <X>' to resolve which idea he means to its slug.", "input_schema": {"type": "object", "properties": {}}},
    {"name": "load_idea_brief", "description": "Load an idea's living brief (its _brief.md — what it is, current thinking, open threads, last riff, sources) plus a live sources index. Call this to CATCH UP on an idea before brainstorming so you are grounded in prior thinking rather than starting cold. Use the slug from list_ideas.", "input_schema": {"type": "object", "properties": {"slug": {"type": "string", "description": "The idea slug from list_ideas (e.g. soul-collective)."}}, "required": ["slug"]}},
    {"name": "capture_idea_riff", "description": "Fold new brainstorm thinking back into an idea's living brief (the CAPTURE step). Call this at the end of a brainstorm exchange to persist what is worth keeping, so the next catch-up sees it. riff = the session's new thinking; thinking (optional) = a durable one-liner for Current thinking.", "input_schema": {"type": "object", "properties": {"slug": {"type": "string", "description": "The idea slug."}, "riff": {"type": "string", "description": "The new thinking from this exchange to capture."}, "thinking": {"type": "string", "description": "Optional durable line to fold into Current thinking."}}, "required": ["slug", "riff"]}},
